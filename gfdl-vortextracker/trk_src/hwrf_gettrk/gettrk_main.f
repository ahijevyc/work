...
c     for the GFS ensemble relocation.
  ...
  93  format (1x,'i= ',i4,' j= ',i4,' glon= ',f7.2,' glat= ',f6.2
     &       ,' dist= ',f8.2,' slp= ',f10.2,' pgradient= ',f9.5)
...
c     igscret  Return code from this subroutine
      ...
      real     xlonsum_sin, xlonsum_cos
      real     xsfclon_sin, xsfclon_cos
...
c      endif
      ...
      if (calcparm(10,ist)) then
        ...
        ipct = ipct + 2
        xlonsum_sin = 2.*sin(clon(ist,ifh,10)*dtr)
        xlonsum_cos = 2.*cos(clon(ist,ifh,10)*dtr)
        xlatsum = 2.*clat(ist,ifh,10)
      endif

      if (calcparm(11,ist)) then
        ! This is for the sfc vorticity center....
        ipct = ipct + 1
        xlonsum_sin = xlonsum_sin + sin(clon(ist,ifh,11)*dtr)
        xlonsum_cos = xlonsum_cos + cos(clon(ist,ifh,11)*dtr)
        xlatsum = xlatsum + clat(ist,ifh,11)
      endif

      if (ipct > 0) then
        xsfclon_sin = xlonsum_sin / float(ipct)
        xsfclon_cos = xlonsum_cos / float(ipct)
        xsfclon = atan2(xsfclon_sin,xsfclon_cos)/dtr
...
c
      ...
      integer ist,ifcsthour,iq,it,maxstorm,iofwret
...
c
      ...
      real      dx,dy,re,ri,ubar,vbar,wgttot,wt_total,barnlat,barnlon
      real      reold,riold,uavg,vavg
      real      dt,barneswt,extrapwt,dtkm,ucomp,vcomp,xdist,ydist
      real      ydeg,extraplat,avglat,cosfac,xdeg,extraplon
      real      ylatdegmove_last, xlondegmove_last,xnumh_last
      real      ylatdegmove_last_perhour, xlondegmove_last_perhour
      real      xnumh_next,yoldavglat,yoldcosfac,xdistmove_last
      real      xdistmove_last_perhour,ynewavglat,ynewcosfac,xdegnew
      real      fixlon(maxstorm,maxtime),fixlat(maxstorm,maxtime)
...
c
      ...
      implicit none
      integer ifret, ifh,ist,iclose,itot4next,ip,iaret,isret,kprm
      integer iwtret1,maxstorm,iwtret2
      real errmax, errinit,xavg_stderr,trkerr_avg,clatsum
      real geslon, geslat, dist, clon_fguess, clat_fguess
      real      clonsum_sin, clonsum_cos
      real      clon_fguess_sin, clon_fguess_cos
...
c     get_uv_center for further details on that).
     ...
     &                    ,clat(ist,ifh,ip),dist,degrees)
          ...
          if (dist <= errmax) then
            iclose = iclose + 1
            clonsum_sin = clonsum_sin + sin(clon(ist,ifh,ip)*dtr)
            clonsum_cos = clonsum_cos + cos(clon(ist,ifh,ip)*dtr)
      ...
      if (iclose > 0) then
        clon_fguess_sin = clonsum_sin/float(iclose)
        clon_fguess_cos = clonsum_cos/float(iclose)
        clon_fguess = atan2(clon_fguess_sin,clon_fguess_cos)/dtr
...
c
      ...
      real    xlon(kmax),wt(kmax), wtot
      real    xwtavg_cos, xwtavg_sin, xwtavg
      ...
      wtot = 0.0
      do i=1,kmax
        xwtavg_cos = xwtavg_cos + cos(xlon(i)*dtr)*wt(i)
        xwtavg_sin = xwtavg_sin + sin(xlon(i)*dtr)*wt(i)
        wtot = wtot + wt(i)
      enddo
c
      if (wtot > 0.0) then
...
c
      ...
      integer npts,ilonfix,jlatfix,ibeg,jbeg,iend,jend,igiret,ist
      integer igucret,igrret,level,nlev,numinterp,imxold,jmxold,i,j
      integer iuo,ip,ivo,iloo,ilao,intnum,ik,imxnew,jmxnew,ibiret
      integer iliret,ivm,ilb,icvret,ifmret,imax,jmax
...
c     Treat longitude as a vector because it is cyclical.
c     Break longitude into sine and cosine components so
c     the components can be averaged and reassembled into the 
c     average longitude vector using atan2(y,x). This handles
c     longitudes that straddle domain boundaries or otherwise
c     are "off" by a multiple of 360 degrees.

      real uvgeslon_sin, uvgeslon_cos, sumlon_sin, sumlon_cos
      real sumlat
...
c     position twice.  
      ...
      sumlon_sin = 2.* sin(guesslon * dtr)
      sumlon_cos = 2.* cos(guesslon * dtr)
      sumlat = 2.*guesslat
      ict = ict + 2
...
c 
            if (ip == 1 .or. ip == 2 .or. ip == 11) then
              ...
              sumlon_cos = sumlon_cos + 2.*cos(clon(ist,ifh,ip)*dtr)
              sumlat = sumlat + 2.*clat(ist,ifh,ip)
              ict = ict + 2
            else
              sumlon_sin = sumlon_sin + sin(clon(ist,ifh,ip)*dtr)
              sumlon_cos = sumlon_cos + cos(clon(ist,ifh,ip)*dtr)
...
c 
      if (ict > 0) then
        uvgeslon_sin = sumlon_sin / ict
        uvgeslon_cos = sumlon_cos / ict
        uvgeslon = atan2(uvgeslon_sin, uvgeslon_cos)/dtr
c       Maybe unnecessary, but to be safe, 
c       put uvgeslon in the same 360-deg longitude range as guesslon,
c       whether it be [-180,180], [180,540], [0,360], or whatever.
c       For example, if guesslon was -120 and uvgeslon is 235,
c       then subtract 360 from uvgeslon.
c       Adding multiples of 360 does not change longitude because
c       it is cyclical.
        uvgeslon = uvgeslon + 360. * NINT((guesslon-uvgeslon)/360.)
...
c     These (i,j) coordinates are returned as ilonfix and jlatfix.
 ...
 623      format (1x,'+++ RPT_GRID_ZETA: n= ',i2,' lev= ',i4
     &         ,' grid zeta= ',f10.7,' igrid zeta= ',i4,' ifilret= ',i3)
...
c
 ...
 31     format (1x,'TIMING: find_maxmin 1  ',i2.2,':',i2.2,':',i2.2)
      ...
      temp_guesslon = temp_guesslon +
     &  360. * NINT((temp_grid_minlon-temp_guesslon)/360.)
      jix = 0
...
c
      ...
      implicit none
      type (trackstuff) trkrinfo
      integer igiret,nhalf,i,npts,jhlatpts,jripts,jbmaxlatpts,imax
      integer jmax,jlatfix,jbeg,jend,ihlonpts,ibmaxlonpts,ilonfix
      integer ibeg,iend
      real geslon,geslat,rdeg,dx,dy,ri,rglatmax,rglatmin,rglonmax
      real rglonmin,cosfac,dlon
      real tmpangle
c
      igiret = 0
c
...
c
      ...
      implicit none
      real degrees,rlatb,rlatc,pole,distlatb,distlatc,difflon,cosanga
      real circ_fract,xdist,rlonb,rlonc
c
...
c Ahijevych had to add .or.iret==9  to get 0.25deg ECMWF grb working
        if (iret == 0.or.iret==9) then
...
c   Ahijevch added .and.iret.ne.9 for ECMWF 0.25 deg grib
      if (iret.ne.0.and.iret.ne.9) then
